...
let getDataFromMongo = async function getDataFromMongo(functionId, parameters) {
    ...
    try {
        ...
        if (functionId === 0) {
            ...
            // let resultObj = await collection.mapReduce(
            //     function() { emit('grp', this.recommendCompany);},
            //     function(key, values) { return Array.sum(values);},      
            //     {  out: { inline: 1 },     
            //      query: {'recommendCompany':{$gt:6}}
            //     })
            // result = resultObj[0].value
            result = 'query'
            console.log(result)
        }
    }
...
function mongoQueryObject(parameters, columnNameIndex, columnName, functionId) {
    ...
    // db.collection('customers').mapReduce(
    //     function() { emit('grp', this.rmr.numerical);},
    //     function(key, rmrValues) { return Array.sum(rmrValues);},      
    //     {  out: { inline: 1 },     
    //      query: {'recommendCompany.numerical':{$gt:6}}
    //     })
    else {
        // let test =eval(db.collection('customers').mapReduce(
        //     function() { emit('grp', this.rmr.numerical);},
        //     function(key, rmrValues) { return Array.sum(rmrValues);},      
        //     {  out: { inline: 1 },     
        //      query: {'recommendCompany.numerical':{$gt:6}}
